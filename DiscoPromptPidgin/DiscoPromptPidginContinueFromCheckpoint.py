...
parser.add_argument("--csv_file_path", type=str, default="/local/musaeed/NPIDRC/BMGF-RoBERTa/data/csvBMGFColumns/pdtb2Pidgin.csv")
...
parser.add_argument("--epoch", type=int, default=1000_000)
...
parser.add_argument("--shouldResumeTrainingFromCheckpoint", type=bool, default=False)
parser.add_argument("--resume_from_checkpoint", type=str, help="Checkpoint to contiune the training from ")
...
parser.add_argument("--percentage", type=float, default=1, help="the percentage of the training data been used in the trainnig ")
...
if args.dataset == "ji":      
    ...
    pdtb2_df = pd.read_csv(args.csv_file_path, low_memory=False)
    ...
    selected_train_df = train_df.sample(frac=float(args.percentage), random_state=42)  
    print(f"length of the full training {len(train_df)}")
    print(f"length of the selected training data is {len(selected_train_df)}")
    
    dataset = {}
    dataset['train'] = Processor.get_examples(selected_train_df, 'train')
...
if args.shouldResumeTrainingFromCheckpoint:
    checkpoint_path = f"{args.resume_from_checkpoint}"
...
for epoch in range(args.epoch):
    ...
    for step, inputs in enumerate(train_dataloader):
        ...
        if actual_step % gradient_accumulation_steps == 0 and glb_step > 0 and glb_step % eval_every_steps == 0:
            ...
            =True)
            output_text = "Glb_step {}, val_acc {}, F1_score {}, Acc_4class {}, F1_score_4class {}, average time {} \n".format(glb_step, val_acc, val_F1_score, val_Acc_score_4class, val_F1_score_4class, tot_train_time / actual_step)
            results_file = args.result_file
            results_file_name = os.path.splitext(results_file)[0]  
            current_datetime = datetime.datetime.now().strftime("%Y-%m-%d_%H-%M-%S")
            results_file_with_datetime = f"{results_file_name}_{current_datetime}.txt"  
