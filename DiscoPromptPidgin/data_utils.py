...
class PDTB2Processor(DataProcessor):
    ...
    def get_examples(self, df, split):
        ...
        for index, row in df.iterrows():
            ...
            for k in ["ConnHeadSemClass1", "ConnHeadSemClass2", "Conn2SemClass1", "Conn2SemClass2"]:
            ...
            if int(row["Arg1_SpanList"].replace("..",",").split(',')[0]) < int(row["Arg2_SpanList"].replace("..",",").split(',')[0]):
                text_a = post_process_arg1(row["Arg1_RawText"])
                text_b = post_process_arg2(row["Arg2_RawText"])
            else:
                text_a = post_process_arg1(row["Arg2_RawText"])
                text_b = post_process_arg2(row["Arg1_RawText"])

            meta = {
                "conn1": row["Conn1"],
                "conn2": row["Conn2"],
                "conn1_senses": [row["ConnHeadSemClass1"], row["ConnHeadSemClass2"]],
                "conn2_senses": [row["Conn2SemClass1"], row["Conn2SemClass2"]],
            ...
            guid = "%d_%s_wsj_%02d%02d" % (
                index, row["Relation"], row["Section"], row["FileNumber"]
            )
            example = InputExample(guid=guid, text_a=text_a, text_b=text_b, meta=meta, label=label) 
...
class PDTB2EXPProcessor(DataProcessor):
    rel_map_4 = {
        ...
        "Contingency.Pragmatic cause.Justification": 1,
        "Contingency.Pragmatic condition.Relevance": 1,
        "Contingency.Condition.Unreal present": 1,  
        "Contingency.Condition.Unreal past": 1, 
        "Contingency.Condition.General": 1,  
        "Contingency.Condition.Factual past": 1,  
        "Contingency.Condition.Factual present": 1, 
        "Contingency.Pragmatic condition.Implicit assertion": 1, 
        "Expansion": 2,
        "Expansion.Alternative": 2,
        "Expansion.Alternative.Chosen alternative": 2,
        "Expansion.Alternative.Conjunctive": 2,
